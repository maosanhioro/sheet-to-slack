...
5) Slackチャンネル（必須）  
   - 例: `#general` や `#news`。先頭に `#` を付けるのが安全です（付いていない場合は自動で付与されます）。チャンネルID `C...` 形式ならそのまま書けます。  
   - シートの表示形式で先頭に `#` が見える設定になっていても、入力時はそのまま書いてください（`#` は自動で付与されます）。
6) 宛先（任意）  
   - 通知にメンションを付けたいときに入力します。カンマで区切れば複数指定できます。  
   - 例: `here` / `channel` / `alice`（@なしの名前） / `U123ABC`（ユーザーID） / `<@U123ABC>`  
   - 名前に `@` を付けて書くと `@@name` になってしまうので、名前は @ なしで入力してください。  
   - カンマの後に改行やスペースが入っても問題ありません。
7) 通知内容（必須）  
   - Slack に送る本文を書きます。
8) 登録者（任意）  
   - エラーで送れなかったとき、ここに書いたユーザーへDM送信を試みます。`@name` でも `U123ABC` でもOKです。  
   - シートの表示形式で先頭に `@` が見える設定になっていても、名前は `@` なしで入力するのが安全です（`@name` と入力すると `@@name` になります）。IDの場合は `U123ABC` または `<@U123ABC>` と書いてください。
