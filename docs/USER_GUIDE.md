...
9) 棚卸し状態 / 棚卸し通知日（通常は手入力しない）  
   - 過去日付の単発通知に対して、整理を促す通知が送られたあとに自動で入ります。  
   - `棚卸し状態` が `通知済み` の行は、「もう予定日を過ぎていて、不要なら消してよい候補」です。  
   - `棚卸し通知日` には、その案内を送った日が入ります。
...
- 年/月/日がすべて入っている単発通知で、予定日を過ぎたものは別トリガーの棚卸し通知の対象になります。
...
- **`棚卸し状態` が `通知済み` のまま残っている** → その行は予定日を過ぎた単発通知です。不要なら削除してください。残すならメモ欄などで運用理由を分かるようにしておくと安全です。

## 5. 困ったときは
- 送られない場合は、必須列（時刻・チャンネル・通知内容）が入っているか、日付/曜日の条件に合っているか、土日祝の設定を確認してください。
- 単発通知で予定日を過ぎた行は、棚卸し通知のあと `棚卸し状態` が `通知済み` になります。不要行の整理に使ってください。
