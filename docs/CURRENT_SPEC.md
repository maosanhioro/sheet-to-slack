...
- 12: Slackチャンネル（必須・先頭に`#`を推奨。ID形式 `C…` ならそのまま）
...
- 宛先は here/channel を `<!here>` 等に変換し、カンマ区切りの各要素をそのまま扱う（`<@ID>` 形式または `U…`/`W…` で始まるIDは `<@id>` に、その他は先頭に `@` を付ける。※`@name` と入力すると `@@name` になるので名前は @ なしで入力する）。
