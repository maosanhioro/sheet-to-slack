...
- 14: 通知内容（必須）
- 15: 登録者（任意）
- 16: 棚卸し状態（任意・期限切れ通知の送信後に `通知済み` が入る）
- 17: 棚卸し通知日（任意・期限切れ通知を送った日付 `yyyy/MM/dd`）
...
- 期限切れ通知: トリガーで `ReportExpired` を実行する前提。過去日付の単発通知のうち、`棚卸し状態` が空の行だけを元の Slack チャンネルへ通知し、送信成功後に `棚卸し状態=通知済み` と `棚卸し通知日` を更新する。
...
- 年月日がすべて指定されていて「過去日付」の行は「期限切れ」としてスキップする。
- `ReportExpired` は、期限切れ行のうち `棚卸し状態` が空の行だけを対象に、元の Slack チャンネルへ「不要なら削除してください」と通知する。
- `ReportExpired` の通知成功後、その行の `棚卸し状態=通知済み`、`棚卸し通知日=実行日` を書き戻す。同じ行は状態を手で空に戻さない限り再通知しない。
...
- 期限切れ通知では宛先列は再利用せず、元の Slack チャンネルのみに送る。
...
- 棚卸し通知を送った行は `棚卸し状態=通知済み` になる。削除しないで残す場合は、そのまま状態を見て目視棚卸しできる。
