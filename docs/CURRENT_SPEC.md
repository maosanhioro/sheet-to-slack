...
- 期限切れ通知: トリガーで `ReportExpired` を実行する前提。通知先 `EXPIRED_REPORT_CHANNEL`（任意。未設定時は管理者メールへ送信、BOT_MASTER がなければログのみ）
...
- 年月日がすべて指定されていて「過去日付」の行は「期限切れ」としてスキップし、`ReportExpired` でまとめて通知（内容はシート名・行番号・指定日付・登録者）。
