...
- 14: 通知内容（必須）
- 15: 登録者（任意）
- 16: 棚卸し記録日（任意・`ReportExpired` が期限切れと判定した日付 `yyyy/MM/dd` が入る。空欄なら未処理）
...
- 期限切れ棚卸し: トリガーで `ReportExpired` を実行する前提。過去日付の単発通知のうち、`棚卸し記録日` が空の行だけを対象に `棚卸し記録日` へ実行日をシートへ記録する（Slack通知は行わない）。
...
- `ReportExpired` は、期限切れ行のうち `棚卸し記録日` が空の行だけを対象に、`棚卸し記録日=実行日` をシートへ書き戻す（Slack通知は行わない）。同じ行は記録日を手で空に戻さない限り再記録しない。
...
- 期限切れとして棚卸しされた行は `棚卸し記録日` に日付が入る。削除しないで残す場合は、条件付き書式（`docs/USER_GUIDE.md` 参照）や記録日列を見て目視で棚卸しできる。
